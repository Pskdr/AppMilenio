...
public class AdapterInsectos extends RecyclerView.Adapter<AdapterInsectos.ViewHolderCliente> {
    ...
    public AdapterInsectos(AdapterInsectos.onItemListener onItemListener, ArrayList<InsectoGroupMostrar> arrayList, Fragment instancia, boolean bloquear) {
    ...
    public void onBindViewHolder(@NonNull ViewHolderCliente holder, int position) {
        ...
        holder.btnBorrar.setOnClickListener(v -> {
            if(instancia instanceof  CrearOrdenServicioFragment) {
                ((CrearOrdenServicioFragment)instancia).eliminarInsecto(insectosArraylist.get(position).getIdInsecto());
            }
            if(instancia instanceof CrearOrdenInspeccionFragment) {
                ((CrearOrdenInspeccionFragment)instancia).eliminarInsecto(insectosArraylist.get(position).getIdInsecto());
            }
        });
    }
